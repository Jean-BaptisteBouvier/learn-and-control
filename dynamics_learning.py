...
Functions to learn the dynamics of a system
"""
...
def rand_data(sys, N, random_actions):
    ...
    for i in range(N):
        ...
        s = torch.randn((1, s_sz)) # normal distribution (0,1)
        if random_actions:
            ...
            if sys.squared_inputs:
                a = torch.sqrt(torch.clamp(-s @ K.t(), min=sys.input_min**2, max=sys.input_max**2))
            else:
...
class Dynamics_NN(nn.Module):
    def __init__(self, sys, num_hiddens = 64):
        ...
        # Training set parameters to be updated
        self.input_mean = 0.
        self.input_std = 0.
        self.output_mean = 0.
        self.output_std = 0.
...
# Network training and plotting training and testing loss
...
def relative_error(model, data, data_name='data'):
    ...
    print(f"Average relative error over {data_name} is {rel_error.mean().item():.2f} % and max relative error is {rel_error.max().item():.2f} %")



# Transient parameters for the linearized inverted pendulum dynamics
def transient_bd_params(sys):
